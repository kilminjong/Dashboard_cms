...
// 고객원장의 이메일(contact_email)만 시트에 반영.
// 서버(Edge Function)가 해당 행을 읽어 이메일 칸만 교체 후 다시 씀 → 다른 컬럼 보존.
export async function updateCustomerEmailInSheet(customer: any, email: string): Promise<boolean> {
  if (!customer || customer._rowIndex == null) {
    throw new Error('행 정보가 없어 시트에 반영할 수 없습니다. 목록을 새로고침 해주세요.')
  }
  const res = await fetch(EDGE_FUNCTION_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ action: 'updateCustomerEmail', rowIndex: customer._rowIndex, email: email || '' }),
  })
  const data = await res.json()
